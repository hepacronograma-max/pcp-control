...
-- Uso: cole este arquivo, ou rode node scripts/apply-packaging-volumes-migration.js
...
COMMENT ON TABLE public.packaging_volumes IS
  'Uma linha = uma etiqueta de embalagem = uma caixa física.';
COMMENT ON COLUMN public.packaging_volumes.qr_token IS
  'Token único da URL de bipagem (/embarque/bipar/{qr_token}).';
COMMENT ON COLUMN public.packaging_volumes.sequence IS
  'Número do volume no pedido (ex.: Volume 3).';

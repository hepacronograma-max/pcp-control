...
function ImportReportSummary({
  ...
  return (
    <div className={`${compact ? "mt-2" : "mt-4"} space-y-4`}>
      ...
      {!compact && isShadowReport(report) && (report.shadow_logs ?? []).length > 0 && (
        <details className="text-xs text-slate-600">
          <summary className="cursor-pointer font-medium">Log shadow ({report.shadow_logs!.length})</summary>
...
function isShadowReport(report: OmieImportReport) {
  return report.modo === "shadow";
}

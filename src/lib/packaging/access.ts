...
export type PackagingAccessErr = { ok: false; error: string; status: number };

/**
 * Autentica o pedido e garante que `companyId` é a empresa do usuário.
 * `requireSettings` restringe a manager/super_admin (CRUD em Configurações).
 */
export async function assertPackagingCompanyAccess(
  companyIdParam: string | null,
  opts: { requireSettings?: boolean } = {}
): Promise<PackagingAccessOk | PackagingAccessErr> {

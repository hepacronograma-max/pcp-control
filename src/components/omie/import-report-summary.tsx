...
export function summarizeOmieImportReport(report: OmieImportReport): {
  ...
  if (report.modo === "shadow") {
    const sim = report.itens_simulados_shadow ?? 0;
    return {
      level: "info",
      title: "Simulação shadow (nada gravado)",
      description:
        sim > 0
          ? `${report.encontrados} pedido(s) na etapa 20 · ${sim} alteração(ões) de item detectadas (shadow, não gravado)`
          : `${report.encontrados} pedido(s) na etapa 20 · modo shadow — nenhuma gravação no PCP`,
    };
  }
    ...
    <div className={`${compact ? "mt-2" : "mt-4"} space-y-4`}>
      {report.modo === "shadow" && (
        <div className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-950">
          <p className="font-medium">Modo shadow — simulação, nada gravado no PCP</p>
          {(report.itens_simulados_shadow ?? 0) > 0 && (
            <p className="mt-1">
              {report.itens_simulados_shadow} alteração(ões) de item detectadas (não gravadas).
            </p>
          )}
        </div>
      )}

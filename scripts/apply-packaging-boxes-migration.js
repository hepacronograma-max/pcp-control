/**
 * Aplica supabase/migrations/20260824_packaging_boxes.sql no projeto remoto.
 * Tenta: SUPABASE_ACCESS_TOKEN (PAT) → Management API, depois DATABASE_URL (pg).
 *
 * Uso: node scripts/apply-packaging-boxes-migration.js
 */
const fs = require("fs");
...
async function verifyTable() {
  ...
  const { error } = await supabase.from("packaging_boxes").select("id").limit(1);
...
async function main() {
  ...
  console.log("Aplicando migration packaging_boxes...");
  ...
  if (!result.ok) {
    ...
    console.error(
      "\nConfigure no .env.local uma das opções:\n" +
        ...
        "Depois rode: node scripts/apply-packaging-boxes-migration.js\n" +
        "Ou cole o SQL manualmente no SQL Editor do Supabase (supabase-packaging-boxes.sql)."
    );
    process.exit(1);
  }
  ...
  console.log(
    ok
      ? "Verificação: tabela packaging_boxes acessível."
      : "Verificação: conferir no SQL Editor."
  );
}
